...
class BattleRoomCubit extends Cubit<BattleRoomState> {
  ...
  void updateState(BattleRoomState newState, {cancelSubscription = false}) {
  ...
  void subscribeToBattleRoom(
      String battleRoomDocumentId, List<Question> questions, bool type) {
    //for realtimeness
...
void joinBattleRoomWithBot(
  ...
  void deleteUserFromRoom(String userId) {
    if (state is BattleRoomUserFound) {
      BattleRoom room = (state as BattleRoomUserFound).battleRoom;
  ...
  bool opponentLeftTheGame(String userId) {
    if (state is BattleRoomUserFound) {
      ...
      print(
          "User submitted answer ${getCurrentUserDetails(userId).answers.length}");
